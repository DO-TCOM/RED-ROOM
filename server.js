...
// ── CONFIG ────────────────────────────────────────────────────────────────────
// URL de base de ton Hostinger (sans slash final)
const HOSTINGER_URL = 'https://chaltet.com';
...
// Cache VPN en mémoire (perdu au redéploiement mais ok)
const vpnCache = {};

async function isVPN(ip) {
  if (vpnCache[ip] && (Date.now() / 1000 - vpnCache[ip].time) < 86400) return vpnCache[ip].is_vpn;
  try {
    ...
    vpnCache[ip] = { is_vpn: isVpn, time: Math.floor(Date.now() / 1000) };
    return isVpn;
  } catch { return false; }
}
...
// ── ENVOYER LOG VERS HOSTINGER ────────────────────────────────────────────────
async function logToHostinger(ip, ua, roomName) {
  try {
    ...
    // On envoie vers une route API sur Hostinger
    await fetch(`${HOSTINGER_URL}/api_log.php`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ip, city: loc.city, country: loc.country, vpn: vpnFlag, browser, date, room: roomName }),
      signal: AbortSignal.timeout(5000)
    });
  } catch (e) { console.error('logToHostinger error:', e.message); }
}

// ── VÉRIFIER BLACKLIST SUR HOSTINGER ─────────────────────────────────────────
async function checkBlacklist(ip) {
  try {
    const res = await fetch(`${HOSTINGER_URL}/api_check.php?ip=${encodeURIComponent(ip)}`, {
...
// ── ROUTE COLLECT (reçoit les infos JS du front et les envoie à Hostinger) ───
app.post('/collect', async (req, res) => {
  try {
    await fetch(`${HOSTINGER_URL}/collect.php`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(req.body),
      signal: AbortSignal.timeout(5000)
    });
  } catch (e) { console.error('collect proxy error:', e.message); }
  res.json({ ok: true });
});
...
io.on('connection', async (socket) => {
  ...
  // Vérifier blacklist sur Hostinger
  const blocked = await checkBlacklist(ip);
  if (blocked) {
    socket.emit('kicked');
  ...
  // Logger vers Hostinger en arrière-plan
  const roomName = socket.handshake.query?.room || 'default';
  logToHostinger(ip, ua, roomName);
